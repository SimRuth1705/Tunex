...
const Footer = () => {
  ...
  // 🌟 Individual element animation (Slide up and fade in)
  const itemVariants = {
  ...
  return (
    <footer className="w-full bg-black border-t border-white/5 pt-12 pb-8 px-6 lg:px-12 overflow-hidden">
      
      {/* 🌟 Apply motion to the main wrapper to trigger animations when in view */}
      <motion.div 
        variants={containerVariants}
        initial="hidden"
        whileInView="show"
        viewport={{ once: true, margin: "-50px" }} // Triggers just before it fully enters the screen
      ...
      >
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
          ...
          <motion.div variants={itemVariants} className="flex flex-col gap-4">
            <Link to="/" className="text-2xl font-black italic tracking-tighter text-white cursor-none!">
          ...
          <motion.div variants={itemVariants} className="grid grid-cols-2 gap-x-12 gap-y-3">
            {[
              { name: 'Home', path: '/' },
              ...
              <Link 
                key={link.name} 
                to={link.path} 
                className="text-[9px] uppercase tracking-widest font-mono text-gray-400 hover:text-[#FF7F11] transition-colors cursor-none! py-1"
              >
                {link.name}
              </Link>
            ))}
          ...
          <motion.div variants={itemVariants} className="flex flex-col items-end gap-4">
            <div className="flex items-center gap-2 px-3 py-1.5 bg-white/5 border border-white/10 rounded-full">
              <div className="w-1.5 h-1.5 rounded-full bg-[#FF7F11] animate-pulse drop-shadow-[0_0_5px_rgba(255,127,17,0.8)]"></div>
        ...
        <motion.div 
          ...
          className="border-t border-white/5 mt-12 pt-6 flex flex-col md:flex-row justify-between items-center gap-4"
        >
